                        ...
                        <!--<p class="division">JANUARY</p>-->
                            <!--<table class="musicTableClass"> 
                             ...
                             <?php 
                                ...
                                $sql = "SELECT m.MusicianId, m.Name, m.Musician_Image_URL, p.PerformanceId, p.Description, p.Month_Year from musician as m, performance as p where m.MusicianId = p.MusicianId group by p.Month_Year desc";
                                $result = mysqli_query($con,$sql);
                                if ($result->num_rows > 0) {
                                    // output data of each row
                                    while($row = $result->fetch_assoc()) {
                                        echo '<p class="division">'.$row["Month_Year"].'</p>';
                                        echo '<div class="rowMusicPhp">';
                                        echo '<div class = "columnMusicPhpImg"><img src="'.$row["Musician_Image_URL"].'"></div>';
                                        echo '<div class = "columnMusicPhp">'.$row["Description"].'</div>';
                                        echo '</div>';
                                    }// end of while
                               } //end of if
                               CloseCon($con);
